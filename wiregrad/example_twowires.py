...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./data/config/text/horse_bull.json', help='path to the input config file')
    ...
    for iter in tqdm(range(iterations), 'gradient descent'):
        ...
        for i, view in enumerate(views):
            ...
            if iter % 10 == 0 or iter == iterations - 1:
                ...
                save_polyline(nodes = points[0*num:1*num].detach(), path = os.path.join(output_dir, 'controls_0.obj'))
                save_polyline(nodes = points[1*num:2*num].detach(), path = os.path.join(output_dir, 'controls_1.obj'))
