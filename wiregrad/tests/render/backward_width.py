...
if __name__ == '__main__':
    ...
    ##   the backward pass of our line rendering (w.r.t. the polyline width) is correct.
    ##
    ...
    stroke_width = torch.tensor([2.0], dtype=torch.float32)
    stroke_width.requires_grad_()
    ...
    print('gradient sum =', stroke_width.grad.item())
    ...
    stroke_width.grad = None

    tmp = wg.render_polylines(
        (width, height),
        mvp = mvp,
        polylines = [ wg.cubic_basis_spline(points, knots=num_knots) ],
        stroke_width = stroke_width,
    ...
    for ih in tqdm(range(height), 'brute-force rendering of a gradient image'):
        for iw in range(width):
            ...
            grad_image[ih, iw] = stroke_width.grad.detach().item()
            stroke_width.grad.zero_()



    ## rendering of a finite-difference image
    delta = 5e-2
    stroke_width = stroke_width.detach()
    stroke_width[0] += delta

    img_delta = wg.render_polylines(
        (width, height),
        mvp = mvp,
        polylines = [ wg.cubic_basis_spline(points, knots=num_knots) ],
        stroke_width = stroke_width,
