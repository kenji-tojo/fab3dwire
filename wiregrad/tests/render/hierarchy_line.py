...
if __name__ == '__main__':
    ##
    ## (Internal) This code tests the hierarchical acceleration of line rendering
    ##
    ...
    start = time.time()
    img_bruteforce = wg.render_polylines(
        (width, height),
        mvp = mvp,
        polylines = [ nodes ],
    ...
    start = time.time()
    img_accel_single = wg.render_polylines(
        (width, height),
        mvp = mvp,
        polylines = [ nodes ],
        stroke_width = stroke_width,
    ...
    start = time.time()
    img_accel_multi = wg.render_polylines(
        (width, height),
        mvp = mvp,
        polylines = [ nodes ],
        stroke_width = stroke_width,
    ...
    if use_cuda:
        ...
        start = time.time()
        img_cuda = wg.render_polylines(
            (width, height),
            mvp = mvp,
            polylines = [ nodes.cuda() ],
            stroke_width = stroke_width,
            cyclic = cyclic,
            num_samples = args.samples,
            use_hierarchy = True
    ...
    imsave.save_image(img_accel_multi, os.path.join(output_dir, 'trefoil_line.png'))
